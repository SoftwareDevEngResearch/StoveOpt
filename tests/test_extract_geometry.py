...
def test_extract_geometry():
    ...
    if file_path == None:
        ...
        print(file_path)
    return file_path
    assert extract_geometry(file_path).pt1x == 0
    assert extract_geometry(file_path).pt1z == 0
    assert extract_geometry(file_path).pt1y == 0
    assert extract_geometry(file_path).pt2x == 0.1
    assert extract_geometry(file_path).pt2z == 0
    assert extract_geometry(file_path).pt2y == 0
    assert extract_geometry(file_path).pt3x == 0
    assert extract_geometry(file_path).pt3z == 0.15
    assert extract_geometry(file_path).pt3y == 0
    assert extract_geometry(file_path).pt4x == 0.1
    assert extract_geometry(file_path).pt4z == 0.15
    assert extract_geometry(file_path).pt4y == 0
    assert extract_geometry(file_path).pt5x == 0.1
    assert extract_geometry(file_path).pt5z == 0.16
    assert extract_geometry(file_path).pt5y == 0
    assert extract_geometry(file_path).pt6x == 0
    assert extract_geometry(file_path).pt6z == 0.16
    assert extract_geometry(file_path).pt6y == 0
    assert extract_geometry(file_path).pt7x == 0
    assert extract_geometry(file_path).pt7z == 0.3
    assert extract_geometry(file_path).pt7y == 0
    assert extract_geometry(file_path).pt8x == 0.1
    assert extract_geometry(file_path).pt8z == 0.3
    assert extract_geometry(file_path).pt8y == 0
    assert extract_geometry(file_path).pt9x == 0.17
    assert extract_geometry(file_path).pt9z == 0.3
    assert extract_geometry(file_path).pt9y == 0
    assert extract_geometry(file_path).pt10x == -0.07
    assert extract_geometry(file_path).pt10z == 0.3
    assert extract_geometry(file_path).pt10y == 0
    assert extract_geometry(file_path).pt11x == -0.07
    assert extract_geometry(file_path).pt11z == 0.5
    assert extract_geometry(file_path).pt11y == 0
    assert extract_geometry(file_path).pt12x == -.04
    assert extract_geometry(file_path).pt12z == 0.5
    assert extract_geometry(file_path).pt12y == 0
    assert extract_geometry(file_path).pt13x == 0.14
    assert extract_geometry(file_path).pt13z == 0.5
    assert extract_geometry(file_path).pt13y == 0
    assert extract_geometry(file_path).pt14x == 0.17
    assert extract_geometry(file_path).pt14z == 0.5
    assert extract_geometry(file_path).pt14y == 0
    assert extract_geometry(file_path).pt15x == -0.04
    assert extract_geometry(file_path).pt15z == 0.33
    assert extract_geometry(file_path).pt15y == 0
    assert extract_geometry(file_path).pt16x == 0.14
    assert extract_geometry(file_path).pt16z == 0.33
    assert extract_geometry(file_path).pt16y == 0
